...
class ComprasModelo
{
    ...
    public static function mdlAllCompras()
    {
        try {
            //code...
            $sql = "SELECT cps.*,pvs.pvs_nombre FROM tbl_compras_cps cps 
            JOIN tbl_proveedores_pvs pvs ON cps.cps_id_proveedor= pvs.pvs_id ORDER BY cps.cps_id DESC";
            $con = ConexionIfixit::conectar();
            $pps = $con->prepare($sql);
            $pps->execute();
    ...
    public static function mdlAutocompleteProductos($pds_nombre)
    {
        try {
            //code...
            $sql = "SELECT productos.id,productos.pds_nombre,CONCAT(productos.codigo,' - ',productos.descripcion) as label, productos.codigo, productos.descripcion, categorias.categoria FROM productos JOIN categorias ON productos.id_categoria = categorias.id WHERE productos.pds_nombre LIKE '%$pds_nombre%' OR productos.codigo LIKE '%$pds_nombre%' OR productos.descripcion LIKE '%$pds_nombre%' OR categorias.categoria LIKE '%$pds_nombre%' ";
            $con = ConexionIfixit::conectar();
            $pps = $con->prepare($sql);
            $pps->execute();
